...
def main(python_file, outdir, name: str, parallelize: bool, gpus: str):
    ...
    launch_id = f"{name}-{datetime.now().strftime('%Y-%m-%d-%H-%M-%S')}"
    ...
    print(f"Running sweep {launch_id} with {len(configs)} configs")
    ...
    if not use_ray:
        ...
        num_gpus = 1 / MAX_WORKERS_PER_GPU
        remote = ray.remote(num_gpus=num_gpus)(execute_config)
