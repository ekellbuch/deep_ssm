...
@torch.no_grad
def compute_gradient_norms(model, square=False):
    grad_norms = {}
...
@torch.no_grad
def compute_param_norms(model, square=False):
    param_norms = {}
    for name, param in model.named_parameters():
        if param.grad is not None:
        ...
        self.logger.log(
            {
                ...
                f"param_norm/{name}": value
                for name, value in param_norms.items()
            } | {"train/lr": self.scheduler.get_last_lr()[0]}  #  if hasattr(self, "scheduler") else {}
