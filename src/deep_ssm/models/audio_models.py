...
from deep_ssm.mixers.s4 import LinearActivation, S4Block as S4
...
class DownPool(nn.Module):
    ...
    def forward(self, x):
        # expects: x: (B, D, L) if transposed
        if self.transposed:
            # Pool and rearrange: (B, D, L) -> (B, D * pool, L // pool)
            x = rearrange(x, '... h (l s) -> ... (h s) l', s=self.pool)
        else:
            # Case when transposed is False: expects x of shape (B, L, D)
            # Pool and rearrange: (B, L, D) -> (B, L // pool, D * pool)
    ...
    def step(self, x, state, **kwargs):
        ...
        x: (..., H) if transposed
        """
        # TODO: fix for self.transposed is False
        if x is None: return None, state
        state.append(x)
        if len(state) == self.pool:
            # Stack the states together along a new dimension (time/sequence dimension)
...
class UpPool(nn.Module):
    ...
    def forward(self, x, skip=None):
        ...
        if self.transposed:
            ...
            x = F.pad(x[..., :-1, :], (0, 0, 1, 0))  # Shift right along the sequence length
    ...
    def step(self, x, state, **kwargs):
        ...
        if len(state) == 0:
            ...
            if self.transposed:
                x = x.squeeze(-1)  # Remove the added sequence dimension
...
class FFBlock(nn.Module):
    ...
    def forward(self, x):
        # expects: (B, D, L) if transposed else (B, L, D)
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        """
        Input x is shape (B, d_input, L) if transposed else (B, L, d_input)
...
class Sashimi(nn.Module):
    def __init__(
        ...
        **ssm_args,
        ...
        ssm_blocks = {"s4": s4_block, "mamba": mamba_block}
        ssm_block = ssm_blocks[ssm_type]
        ...
        for p in pool:
            if unet:
                ...
                for _ in range(n_layers):
                    d_layers.append(ssm_block(H))
                    if ff > 0: d_layers.append(ff_block(H))

            # Add sequence downsampling and feature expanding
            d_layers.append(DownPool(H, expand, p, self.transposed))
            H *= expand
        ...
        for p in pool[::-1]:
            block = []
            H //= expand
            block.append(UpPool(H * expand, expand, p, self.transposed))

            for _ in range(n_layers):
                block.append(ssm_block(H))
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        ys = []
        state = model.default_state(*x.shape[:1])
